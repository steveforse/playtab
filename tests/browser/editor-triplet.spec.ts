...
test('ED-11 creates and removes a paired triplet with one undo step each', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Selection event' }).locator('option')).toHaveCount(5);
  ...
  await expect(page.getByRole('combobox', { name: 'Selection event' }).locator('option')).toHaveCount(3);
