...
The native path supports legacy TEF2 and modern TablEdit TEF 3.00 files with one five-string track, bounded file size and measure count, source time signatures, alternate tunings, and the currently decoded TEF annotations and note effects. Unsupported or malformed files return an error without replacing the current score. Techniques may be incomplete; conversion warnings remain visible. TEF uploads are not yet saved to the library, and TEF export is not implemented.
...
Import an uncompressed `.musicxml` or `.xml` file containing one five-string tablature part (up to 2 MB and 256 measures). Preview supports explicit alternate tuning and offers playback, MIDI, printing, and download of the original XML. It cannot yet be saved in the library or exported to the restricted JSON/plaintext format. Verified duplicate notation/TAB staves are combined for playback; independent staff music is rejected.
