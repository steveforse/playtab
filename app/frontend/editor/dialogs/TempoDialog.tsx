...
export type TempoDialogTarget = { selection: { measure: number; event: number }; info: LocalTempoInfo };
...
export function TempoDialog({ target, onApply, onClose, returnFocus }: {
  ...
  return <dialog ref={ref} className="duplicate-dialog" aria-label="Set tempo here" onCancel={event => { event.preventDefault(); onClose(); }}>
    {target && <>
      ...
      {target.info.opening ? <p>The first event uses the opening tempo ({target.info.local ?? target.info.inherited} BPM). Change it in Score settings.</p> : <>
        <p>Measure {target.selection.measure}, event {target.selection.event}. {target.info.local !== null
