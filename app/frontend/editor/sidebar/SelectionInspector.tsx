...
export type SelectionDetails = { offset: string; pitch: string; grace: { options: { label: string; beat: number }[] } | null };
export type MoveOutcome = { destination: number; fret: number; pitch: string; reason: string | null };
type Navigation = Partial<Pick<ScoreSelection, 'measure' | 'beat' | 'voice' | 'string'>>;

// Where the selection is, how to move it, and the fret and string edits
// for the selected position.
export function SelectionInspector({ selection, details, measureCount, beatCount, onNavigate, fretDraft, onFretDraft,
  moveString, onMoveString, moveMode, onMoveMode, moveOutcome, commands }: {
  selection: ScoreSelection; details: SelectionDetails | null; measureCount: number; beatCount: number; onNavigate: (changes: Navigation) => void;
  ...
  return <>
    <div className="properties-heading">
      ...
      <span className="properties-where">Measure {selection.measure} · Beat {selection.beat}</span>
    </div>
    <div className="editor-selection-summary" aria-live="polite">
      <span>Measure {selection.measure}</span>
      <span>Beat {selection.beat}</span>
    ...
    <div className="editor-selection-fields" aria-label="Go to">
      ...
      <label>Beat<select aria-label="Selection beat" value={selection.beat} onChange={event => onNavigate({ beat: Number(event.target.value) })}>{Array.from({ length: Math.max(1, beatCount) }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label>
      ...
      {details?.grace && <label>Grace<select aria-label="Selection grace" value={selection.beat}
        onChange={event => onNavigate({ beat: Number(event.target.value) })}>
        {details.grace.options.map(option => <option key={option.beat} value={option.beat}>{option.label}</option>)}</select></label>}
    </div>
