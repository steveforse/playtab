// Grace groups: inspection, rewriting, and removal of grace notes.
...
// Removes one string of a grace note, or the whole beat when no string is
// given (or its last string is selected). Grace notes own no measure time, so
// no rest is left behind; the group disappears with its final beat.
export function removeMusicXmlGrace(source: string, score: model.Score, position: RemovalPosition): GraceRemoval {
  ...
  if (!isGrace(target) || members(target) !== rendered.notes.map(note => `${6 - note.string}:${note.fret}`).sort().join('|')) {
    throw new Error('The source grace note does not match the selected grace note.');
  }
  ...
  const wholeBeat = selected.length === target.length;
  const groupRemoved = wholeBeat && !isGrace(groups[position.beat - 1]) && !isGrace(groups[position.beat + 1]);
...
// A null denominator is an imported grace note without a <type>; it stays
// unwritten so opening and applying the dialog never normalizes it.
export type GraceBeatSpec = { denominator: 8 | 16 | null; notes: GraceNoteSpec[] };

// A grace group before its beat (played just before it), or after it (an
// after-grace: played at the end of the beat, e.g. into the barline).
export type GracePlacement = 'before' | 'after';

// `destination` is the ordinary beat the group belongs to; `first` is the
// index of the group's first grace note.
export type GraceGroupInfo = { destination: number; first: number; placement: GracePlacement; graceBeats: GraceBeatSpec[]; readOnly: string[]; connections: string[] };
...
export function locateGraceGroup(source: string, score: model.Score, position: RhythmPosition, readOnly = false, placement: GracePlacement = 'before') {
  ...
  // A selected grace note leads to the ordinary beat its group belongs to.
  ...
  if (!measure || !rendered || rendered.graceType || rendered.isRest) {
    throw new Error(placement === 'before' ? 'Select an ordinary sounding beat as the grace destination.' : 'Select an ordinary sounding beat to add grace notes after it.');
  }
  const tabStaff = sourceTabStaff(document);
...
// Where the grace group containing a grace note sits, or null for an
// ordinary beat.
export function graceBeatPlacement(source: string, score: model.Score, position: RhythmPosition): GracePlacement | null {
...
// Reads each grace note. For a group before a beat, a note's transition
...
// previous note on its string (the main beat or an earlier grace).
export function readGraceBeats(located: ReturnType<typeof locateGraceGroup>) {
  const { lanes, first, end, destination, tabStaff, placement } = located;
  const tabGroups = lanes[0].groups;
  const readOnly: string[] = [];
  const graceBeats: GraceBeatSpec[] = [];
  const lastInScope = placement === 'before' ? destination : end - 1;
  const firstInScope = placement === 'before' ? first : destination;
  const nextOnString = (beatIndex: number, string: number) => {
    for (let index = beatIndex + 1; index <= lastInScope; index++) {
      const match = tabGroups[index].find(note => tabStringOf(note) === string);
      if (match) return match;
    }
    return null;
  };
  const previousOnString = (beatIndex: number, string: number) => {
    for (let index = beatIndex - 1; index >= firstInScope; index--) {
      const match = tabGroups[index].find(note => tabStringOf(note) === string);
      if (match) return match;
    }
    return null;
  ...
  for (let index = first; index < end; index++) {
    const label = `Grace note ${index - first + 1}`;
    ...
    graceBeats.push({ denominator, notes });
  }
  return { graceBeats, readOnly: [...new Set(readOnly)] };
}
...
export function inspectMusicXmlGraceGroup(source: string, score: model.Score, position: RhythmPosition, placement: GracePlacement = 'before'): GraceGroupInfo {
  ...
  const { graceBeats, readOnly } = readGraceBeats(located);
  const existing = located.lanes.flatMap(lane => lane.groups.slice(located.first, located.end).flat());
  return { destination: located.destination, first: located.first, placement, graceBeats: graceBeats, readOnly, connections: attachedDependencies(existing) };
}

export function validateGraceBeats(graceBeats: GraceBeatSpec[]) {
  if (!graceBeats.length || graceBeats.length > 8) throw new Error('A grace group needs one to eight grace notes.');
  for (const graceBeat of graceBeats) {
    if (graceBeat.denominator !== 8 && graceBeat.denominator !== 16 && graceBeat.denominator !== null) throw new Error('Grace display duration must be 1/8 or 1/16.');
    if (!graceBeat.notes.length || graceBeat.notes.length > 5 || graceBeat.notes.some(member => !Number.isInteger(member.string)
      || member.string < 1 || member.string > 5 || !Number.isInteger(member.fret) || member.fret < 0 || member.fret > 36)
      || new Set(graceBeat.notes.map(member => member.string)).size !== graceBeat.notes.length) {
      throw new Error('A grace note needs one to five distinct strings with frets from 0 to 36.');
    }
    if (graceBeat.notes.some(member => !GRACE_TRANSITIONS.includes(member.transition))) throw new Error('Choose None, Hammer-on, Pull-off, or Slide for each grace transition.');
  }
}

// Replaces the supported grace group before or after an ordinary beat (or
// creates one). Everything is validated on a scratch document, so an invalid
// transition leaves the source untouched.
export function applyMusicXmlGraceGroup(source: string, score: model.Score, position: RhythmPosition, graceBeats: GraceBeatSpec[],
  placement: GracePlacement = 'before'): string {
  validateGraceBeats(graceBeats);
  ...
  const current = readGraceBeats(located);
  ...
  const graceStrings = new Set(graceBeats.flatMap(graceBeat => graceBeat.notes.map(member => member.string)));
  ...
  const created = lanes.map((lane, laneIndex) => {
    // Before: ahead of the destination. After: right behind the main beat.
    const destinationGroup = lane.groups[destination];
    let cursor: Element = destinationGroup[destinationGroup.length - 1];
    return graceBeats.map(graceBeat => graceBeat.notes.map((member, index) => {
      ...
      if (graceBeat.denominator) setText(note, 'type', graceBeat.denominator === 8 ? 'eighth' : '16th');
  ...
  graceBeats.forEach((graceBeat, beatIndex) => graceBeat.notes.forEach((member, memberIndex) => {
    if (member.transition === 'none') return;
    const where = `Grace note ${beatIndex + 1}, string ${member.string}`;
    const label = TRANSITION_LABELS[member.transition];
    const grace = created.map(lane => lane[beatIndex][memberIndex]);
    // The other end: before a destination it is the next note on the string;
    // after a beat it is the previous one.
    let other: Element[] | null = null;
    let otherFret = 0;
    const step = placement === 'before' ? 1 : -1;
    for (let at = beatIndex + step; at >= 0 && at < graceBeats.length && !other; at += step) {
      const index = graceBeats[at].notes.findIndex(candidate => candidate.string === member.string);
      if (index >= 0) { other = created.map(lane => lane[at][index]); otherFret = graceBeats[at].notes[index].fret; }
    }
    if (!other) {
    ...
    if (!other) {
      throw new Error(placement === 'before'
        ...
        : `${where}: the ${label} needs a note on string ${member.string} in the main beat or an earlier grace note.`);
    }
...
export function addMusicXmlGraceGroup(source: string, score: model.Score, position: RhythmPosition,
  ...
  const rendered = score.tracks?.[0]?.staves?.[0]?.bars?.[position.measure]?.voices?.[position.voice]?.beats?.[position.beat];
  if (!rendered || rendered.graceType || rendered.isRest) throw new Error('Select an ordinary sounding beat as the grace destination.');
...
export function removeMusicXmlGraceGroup(source: string, score: model.Score, position: RhythmPosition,
  ...
  if (first === end) throw new Error('This beat has no grace group to remove.');
