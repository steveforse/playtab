...
type Props = {
  ...
  onRestart: () => void; onPlayPause: () => void; top?: boolean;
};
const seconds = (milliseconds: number) => `${Math.floor(milliseconds / 60000)}:${String(Math.floor(milliseconds / 1000) % 60).padStart(2, '0')}`;

export function PlaybackTransport({ ready, playing, currentTime, endTime, onRestart, onPlayPause, top = false }: Props) {
  const suffix = top ? ' (top)' : '';
  return <div className="transport" role="group" aria-label={top ? 'Top playback controls' : 'Bottom playback controls'}>
