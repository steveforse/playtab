...
`@coderline+alphatab+1.8.4.patch` changes the ESM renderer consumed by this app so hammer-on and pull-off tablature slurs both arch upward, regardless of string. It also gives H/PO pairs a dedicated MIDI path: the origin sample is held on an isolated channel, receives an immediate per-note pitch step at the destination onset, and gets a short, quiet attack transient from the selected SoundFont. This preserves the held legato voice without adding a second picked attack or a slide glyph. Ordinary slides retain the upstream placement and interpolated playback. Native left-hand thumb fingerings are drawn as uppercase `T` markers in a shared band below the tablature staff because this preview hides alphaTab's standard notation staff.
